...
namespace Microsoft.EntityFrameworkCore.Relational.Query.Pipeline.SqlExpressions
{
    public class SelectExpression : TableExpressionBase
    {
        ...
        public CollectionShaperExpression AddCollectionProjection(ShapedQueryExpression shapedQueryExpression, INavigation navigation)
        {
            ...
            _pendingCollectionJoins.Add((GetIdentifyingProjection(),
                innerSelectExpression.GetIdentifyingProjection(),
                innerSelectExpression));

            return new CollectionShaperExpression(
                new ProjectionBindingExpression(this, _pendingCollectionJoins.Count, typeof(object)),
        ...
        {
            var snapshot = _pendingCollectionJoins[collectionId - 1];
            ...
            {
                ApplyThenBy(new OrderingExpression(orderingKey, ascending: true));
            }

            if (collectionId > 1)
            {
                foreach (var orderingKey in _pendingCollectionJoins[collectionId - 2].innerKey)
                {
                    ApplyThenBy(new OrderingExpression(orderingKey, ascending: true));
                }

                outerKey = ConvertKeyExpressions(snapshot.outerKey.Concat(_pendingCollectionJoins[collectionId - 2].innerKey).ToList());
            }
            ...
            if (outer != null)
            {
                ...
                return new RelationalCollectionShaperExpression(
                    new ProjectionBindingExpression(this, collectionId, typeof(object)),
                    outerKey,
                    innerKey,
                    innerShaper,
                    navigation);
